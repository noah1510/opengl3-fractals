...
#ifndef OPENGL_FRACTALS_KOCH_SNOWFLAKE_HPP
#define OPENGL_FRACTALS_KOCH_SNOWFLAKE_HPP
...
protected:
    static const std::vector<glm::vec2> initialLines;
    ...
    static std::vector<glm::vec2> step(const std::vector<glm::vec2>& points, unsigned int depth){
        ...
        for(unsigned int i = 0; i < old_points.size(); i++){
            ...
            glm::vec2 r1 = v1 + delta;
            glm::vec2 r2 = v1 + delta + delta;

            new_points.push_back(old_points[i]);
            new_points.push_back(r1);
            new_points.push_back(r1+helper::rotate(delta,60.0f));
...
#endif //OPENGL_FRACTALS_KOCH_SNOWFLAKE_HPP
